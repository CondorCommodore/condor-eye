...
pub fn compare_books(extracted: &ExtractionResult, truth: &DepthSnapshot) -> ComparisonReport {
    ...
    for (side_name, truth_levels, extracted_levels) in [
        ...
        for t in truth_levels {
            let found = extracted_levels
                .iter()
                .find(|e| (e.price - t.price).abs() < tolerance);
            match found {
                None => {
        ...
        for e in extracted_levels.iter() {
            let in_truth = truth_levels
                .iter()
                .any(|t| (e.price - t.price).abs() < tolerance);
            if !in_truth {
...
mod tests {
    ...
    fn make_extraction(
        bids: Vec<(f64, Option<u64>)>,
        asks: Vec<(f64, Option<u64>)>,
    ) -> ExtractionResult {
    ...
    fn volume_mismatch_detected() {
        let truth = make_truth(vec![(100.50, 500)], vec![(100.51, 400)]);
        let ext = make_extraction(vec![(100.50, Some(999))], vec![(100.51, Some(400))]);
        let report = compare_books(&ext, &truth);
    ...
    fn missing_level_detected() {
        let truth = make_truth(vec![(100.50, 500), (100.49, 300)], vec![(100.51, 400)]);
        let ext = make_extraction(vec![(100.50, Some(500))], vec![(100.51, Some(400))]);
        let report = compare_books(&ext, &truth);
    ...
    fn obscured_volume_not_counted_as_mismatch() {
        let truth = make_truth(vec![(100.50, 500)], vec![(100.51, 400)]);
        let ext = make_extraction(vec![(100.50, None)], vec![(100.51, Some(400))]);
        let report = compare_books(&ext, &truth);
        assert!(
            report.mismatches.is_empty(),
            "obscured volume should not be a mismatch"
        );
    }
    ...
    fn many_missing_causes_fail() {
        ...
        let ext = make_extraction(vec![], vec![(100.51, Some(400))]);
        let report = compare_books(&ext, &truth);
