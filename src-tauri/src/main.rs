...
async fn load_grid_config() -> Result<serde_json::Value, String> {
    ...
    let data = std::fs::read_to_string(&path)
        .map_err(|e| format!("No saved grid config: {}", e))?;
    serde_json::from_str(&data)
        .map_err(|e| format!("Parse grid config: {}", e))
}
...
fn main() {
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            let min_shortcut = Shortcut::new(
                Some(Modifiers::CONTROL | Modifiers::SHIFT),
                Code::KeyM,
            );
            let min_handle = app.handle().clone();
            if let Err(e) = app.global_shortcut().on_shortcut(min_shortcut, move |_app, _shortcut, _event| {
            ...
            let ct_shortcut = Shortcut::new(
                Some(Modifiers::CONTROL | Modifiers::SHIFT),
                Code::KeyT,
            );
            let ct_handle = app.handle().clone();
            if let Err(e) = app.global_shortcut().on_shortcut(ct_shortcut, move |_app, _shortcut, _event| {
